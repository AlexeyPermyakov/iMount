...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НадписьРазмещение = "Запрос на размещение " + Объект.КодЗапросаНаРазмещение;
КонецПроцедуры
...
Процедура НастройкаВидимостиФормыПриИзменении(Элемент)
	...
	Если НастройкаВидимостиФормы = "ПоказатьВсе" Тогда
		ИменаСворачиваемыхГрупп = ИменаСворачиваемыхГруппВсе();
		СкрытьРаскрытьВсеГруппы(Ложь, ИменаСворачиваемыхГрупп);
	ИначеЕсли НастройкаВидимостиФормы = "ПоказатьОсновные" Тогда
		ИменаСворачиваемыхГрупп = ИменаСворачиваемыхГруппОсновные();
		СкрытьРаскрытьВсеГруппы(Ложь, ИменаСворачиваемыхГрупп);
	Иначе
		ИменаСворачиваемыхГрупп = ИменаСворачиваемыхГруппВсе();
		СкрытьРаскрытьВсеГруппы(Истина, ИменаСворачиваемыхГрупп);
	КонецЕсли;
...
Процедура РассчитатьСтоимостьРазмещения(Команда)
	...
	Элементы["ГруппаСтоимостьРазмещенияПоЗапросу"].Показать();
...
&НаКлиенте
Процедура СкрытьРаскрытьВсеГруппы(Свернуть, ИменаСворачиваемыхГрупп)
...
&НаКлиенте
Функция ИменаСворачиваемыхГруппВсе()
	
	ИменаСворачиваемыхГрупп = Новый Массив;
	
	ИменаСворачиваемыхГрупп.Добавить("ГруппаОбщиеСведения");
	ИменаСворачиваемыхГрупп.Добавить("ГруппаСведенияОСделке");
	ИменаСворачиваемыхГрупп.Добавить("ГруппаОбъектИзЗапроса");
	ИменаСворачиваемыхГрупп.Добавить("ГруппаЭлектроснабжение");
	ИменаСворачиваемыхГрупп.Добавить("ГруппаИнформацияОКлиенте");

	Возврат ИменаСворачиваемыхГрупп;
КонецФункции

&НаКлиенте
Функция ИменаСворачиваемыхГруппОсновные()
...
Процедура УправлениеФормой(Форма)
	...
	Если Форма.Объект.ПрименяетсяНаценка Тогда
		Форма.Элементы.СпецСкидкаРасчет.ТолькоПросмотр	= Истина;
		Форма.Элементы.СпецСкидкаСумма.ТолькоПросмотр	= Истина;
	Иначе
		Форма.Элементы.СпецСкидкаРасчет.ТолькоПросмотр 	= Ложь;
		Форма.Элементы.СпецСкидкаСумма.ТолькоПросмотр 	= Ложь;
	КонецЕсли;
	
	Если Форма.Объект.ПрименяетсяСпецСкидка Тогда
		Форма.Элементы.НаценкаРасчет.ТолькоПросмотр	= Истина;
		Форма.Элементы.НаценкаСумма.ТолькоПросмотр	= Истина;
	Иначе
		Форма.Элементы.НаценкаРасчет.ТолькоПросмотр	= Ложь;
		Форма.Элементы.НаценкаСумма.ТолькоПросмотр	= Ложь;
	КонецЕсли; 
	...
	Если Форма.КатегорияАМС = ПредопределенноеЗначение("Перечисление.УБХКатегорииАМС.Золотая") Тогда
		Форма.Элементы.СтатусКатегории.Видимость = Истина;
	Иначе
		Форма.Элементы.СтатусКатегории.Видимость = Ложь;
	КонецЕсли;
