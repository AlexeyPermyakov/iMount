...
//      * id - идентификатор внешнего объекта по данным 1С:Документооборота;
//      * type - тип внешнего объекта по данным 1С:Документооборота.
//
// Возвращаемое значение:
...
Функция ПодходящийТип(ТипОбъектаXDTO, ТипыКандидаты) Экспорт
	...
	ИнтеграцияС1СДокументооборотПереопределяемый.ПриОпределенииПодходящегоТипа(
		ТипОбъектаXDTO, ТипыКандидаты, ПодходящийТип);
...
//	КонецЕсли;
//
Функция СостояниеСогласованияВ1СДокументооборот(СсылкаНаОбъект) Экспорт
	
	Согласован = Ложь;
	
	ИнтеграцияС1СДокументооборотПереопределяемый.ПриОпределенииСостоянияСогласованияВ1СДокументооборот(
		СсылкаНаОбъект, Согласован);
...
//   ТаблицаКонстант - ТаблицаЗначений - см. ИнтеграцияС1СДокументооборот.ЗависимостиКонстант()
//   ИмяРодительскойКонстанты - Строка - имя родительской константы как оно задано в конфигураторе
//   ЗначениеРодительскойКонстанты - Булево - значение родительской константы
//
...
//
// Параметры:
//   ПроцессТип - тип процесса.
//   ПолноеИмя - полное имя точки маршрута
//
// Возвращаемое значение:
...
Функция КраткоеИмяТочкиМаршрута(ПроцессТип, ПолноеИмя) Экспорт
	...
	Если ПроцессТип = "DMBusinessProcessApproval" Тогда
		Если ПолноеИмя = НСтр("ru = 'Ознакомиться с результатом согласования'") Тогда
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessOrder" Тогда
		Если ПолноеИмя = НСтр("ru = 'Контролировать поручение'") Тогда
			КраткоеИмя = "Контролировать";
		ИначеЕсли ПолноеИмя = НСтр("ru = 'Проверить поручение'") Тогда
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessPerformance" Тогда
		Если ПолноеИмя = НСтр("ru = 'Ответственное исполнение'") Тогда
			КраткоеИмя = "ОтветственноеИсполнение";
		ИначеЕсли ПолноеИмя = НСтр("ru = 'Контролировать исполнение'") Тогда
			КраткоеИмя = "Контролировать";
		ИначеЕсли ПолноеИмя = НСтр("ru = 'Проверить исполнение'") Тогда
			КраткоеИмя = "Проверить";
		Иначе
			КраткоеИмя = ПолноеИмя;
		КонецЕсли;
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessRegistration" Тогда
		Если ПолноеИмя = НСтр("ru = 'Ознакомиться с регистрацией'") Тогда
			КраткоеИмя = "Ознакомиться";
		Иначе
			КраткоеИмя = ПолноеИмя;
		КонецЕсли;
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessConfirmation" Тогда
		Если Найти(ПолноеИмя, НСтр("ru = 'Ознакомиться с результатом утверждения'")) <> 0 Тогда
			КраткоеИмя = "Ознакомиться";
		Иначе
			КраткоеИмя = ПолноеИмя;
		КонецЕсли;
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessConsideration" Тогда
		Если ПолноеИмя = НСтр("ru = 'Обработать резолюцию'") Тогда
			КраткоеИмя = "Ознакомиться";
		Иначе
			КраткоеИмя = ПолноеИмя;
		КонецЕсли;
	...
	ИначеЕсли ПроцессТип = "DMComplexBusinessProcess" Тогда
		Если ПолноеИмя = НСтр("ru = 'Контролировать ход процесса'") Тогда
			КраткоеИмя = "Контролировать";
		КонецЕсли;
		
	ИначеЕсли ПроцессТип = "DMBusinessProcessIncomingDocumentProcessing" Тогда
		Если ПолноеИмя = НСтр("ru = 'Исполнение \ ознакомление'") Тогда
			КраткоеИмя = "ИсполнениеОзнакомление";
		ИначеЕсли ПолноеИмя = НСтр("ru = 'В дело'") Тогда
			КраткоеИмя = "ВДело";
		Иначе
			КраткоеИмя = ПолноеИмя;
		КонецЕсли;
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessInternalDocumentProcessing" Тогда
		Если ПолноеИмя = НСтр("ru = 'Рассмотрение'") Тогда
			КраткоеИмя = "ВложенныйПроцесс1";
		ИначеЕсли ПолноеИмя = НСтр("ru = 'Исполнение \ ознакомление'") Тогда
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessIssuesSolution" Тогда
		Если ПолноеИмя = НСтр("ru = 'Рассмотреть вопрос'") Тогда
			КраткоеИмя = "РассмотрениеИнициатором";
		ИначеЕсли ПолноеИмя = НСтр("ru = 'Ознакомиться с результатом рассмотрения'") Тогда
			КраткоеИмя = "ОзнакомлениеСРезультатомРассмотрения";
		КонецЕсли;
		
	ИначеЕсли ПроцессТип = "DMBusinessProcessInvitation" Тогда
		Если ПолноеИмя = НСтр("ru = 'Пригласить'") Тогда
			КраткоеИмя = "Пригласить";
		ИначеЕсли ПолноеИмя = НСтр("ru = 'Ознакомиться с результатом приглашения'") Тогда
			КраткоеИмя = "Ознакомиться";
		ИначеЕсли ПолноеИмя = НСтр("ru = 'Оповестить о результатах приглашения'") Тогда
...
//
// Параметры:
//   Свойства - Структура - См. ИнтеграцияС1СДокументооборот.СтруктураРеквизитовЗаполняемогоВидаДокументовДО.
//
// Возвращаемое значение:
...
//
// Параметры:
//   Свойства - Структура - См. ИнтеграцияС1СДокументооборот.СтруктураРеквизитовЗаполняемойПапкиВнутреннихДокументов.
//
// Возвращаемое значение:
